package IspitJuni.Generics.Z22I;
    ...
    public static <I, O> void process(List<I> inputs, List<Rule<I, O>> rules) {
        for (I input : inputs) {
            ...
            for (Rule<I, O> rule : rules) {
                Optional<O> optional = rule.apply(input);
                if (optional.isEmpty()) {
    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            ...
            rules.add(new Rule<>(r -> r.grades.size() >= 3, r -> r.grades.stream().max(Integer::compare).get().doubleValue()));
