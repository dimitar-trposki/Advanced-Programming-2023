package IspitSeptemvri.Kolokviumski.K5;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    Map<String, Set<T>> genericCollection;

    public GenericCollection() {
        this.genericCollection = new TreeMap<>();
    }

    public void addGenericItem(String category, T element) {
        genericCollection.putIfAbsent(category, new TreeSet<>(Comparator.reverseOrder()));
        genericCollection.get(category).add(element);
    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return genericCollection.values().stream()
    ...
    public Collection<T> itemsFromCategories(List<String> categories) {
        Set<T> toReturn = new TreeSet<>(Comparator.reverseOrder());
        for (Map.Entry<String, Set<T>> entry : genericCollection.entrySet()) {
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        return genericCollection.values().stream()
                ...
                .collect(Collectors.groupingBy(
                        v -> String.format("%02d-%02d", v.getTimestamp().getMonth().getValue(), v.getTimestamp().getDayOfMonth()),
                        TreeMap::new,
