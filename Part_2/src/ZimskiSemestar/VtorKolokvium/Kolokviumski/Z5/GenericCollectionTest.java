package ZimskiSemestar.VtorKolokvium.Kolokviumski.Z5;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    public void addGenericItem(String category, T element) {
        items.putIfAbsent(category, new TreeSet<>(Comparator.reverseOrder()));
        items.get(category).add(element);
    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return items.values().stream()
                .flatMap(v -> v.stream().filter(e -> e.getTimestamp().isAfter(from) && e.getTimestamp().isBefore(to)))
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toCollection(ArrayList::new));
    }

    public Collection<T> itemsFromCategories(List<String> categories) {
        return items.keySet().stream().filter(categories::contains)
                .flatMap(v -> items.get(v).stream())
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        return items.keySet().stream()
                .flatMap(k -> items.get(k).stream())
                .collect(Collectors.groupingBy(
                        c -> String.format("%02d-%02d", c.getTimestamp().getMonth().getValue(), c.getTimestamp().getDayOfMonth()),
                        TreeMap::new,
    ...
    public Map<Integer, Long> countByYear() {
        Map<Integer, Long> toReturn = new TreeMap<>();
        items.values().stream()
                .flatMap(Collection::stream)
                .sorted(Comparator.reverseOrder())
                .forEach(v -> {
                    toReturn.putIfAbsent(v.getTimestamp().getYear(), 0L);
                    toReturn.computeIfPresent(v.getTimestamp().getYear(), (key, val) -> val + 1);
                });
        return toReturn;
    }
}
